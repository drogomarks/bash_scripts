...
sub vcl_recv {
    ...
    if (req.method ~ "GET|HEAD" && req.url ~ "(?i)\.(htm|html)(\?.*|)$") {
        set req.http.static-asset = true;
    }
 
    #-- Cache these requests: GETs should NOT be "passed" to XMLRPC only POSTs
    if (req.method ~ "GET|HEAD" && req.url ~ "(xmlrpc.php|wlmanifest.xml)") {
        unset req.http.cookie;
        return(hash);
    }
...
sub vcl_backend_response {
    ...
    if (beresp.http.Vary ~ "User-Agent"
        && bereq.http.remove-vary-useragent == "true") 
       {
   ...
   # }
    ...
    if (beresp.status == 404) {
        ...
        if (bereq.http.static-asset == "true"
            && bereq.http.allow-cache-404s == "true") {
    ...
    if (beresp.status >= 400) {
      ...
      if (bereq.http.allow-caching-all-other-errors == "true") {
    ...
    if (bereq.http.allow-cache-homepage == "true" && bereq.url ~ "^/$") {
        unset beresp.http.set-cookie;
    }
    ...
    if (beresp.http.Cache-Control ~ "(?i)max-age=3,"
          && bereq.http.wp-supercache-fix == "true") {
    ...
    if (!(bereq.http.x-pass == "true" || bereq.http.admin-url == "true")) {
        unset beresp.http.set-cookie;
    }
}
